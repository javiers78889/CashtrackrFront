...
export default function DeleteBudgetModal() {
    ...
    const deleteBudgetId = params.get('deleteBudgetId')
    const show = deleteBudgetId ? true : false

    const hide = new URLSearchParams(params.toString())
    hide.delete('deleteBudgetId')

    return (
        <>
            <>
                <Transition appear show={show} as={Fragment}>
                    <Dialog as="div" className="relative z-10" onClose={() => router.replace(`${pathname}?${hide}`)}>
                        <TransitionChild
                            as={Fragment}
